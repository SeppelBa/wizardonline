...
## Neu in dieser Runde (Auto-Cleanup & Host-Löschbutton)
- **Auto-Cleanup von Räumen (ohne Server):** Clients löschen automatisch Räume, sobald folgende Regeln zutreffen – ohne Cloud Functions, opportunistisch immer dann, wenn auf der Startseite die Raumliste empfangen wird:
  - **24h ohne Aktivität** (`now - max(updatedAt, listedAt, createdAt) > 24h`) -> Raum wird gelöscht.
  - **Leerer Raum** (keine echten Spieler, nur Bots zählen nicht) -> in Lobby/finished nach **60 Sekunden** Schonfrist, mid-game erst, wenn `updatedAt` deutlich älter als die 2-Minuten-Rejoin-Frist ist.
  - **Beendete Räume** (`phase === "finished"`) -> nach einer **Schonfrist von 30 Minuten** (`FINISHED_ROOM_GRACE_MS`), damit der Gewinner-Screen kurz sichtbar bleibt.
  - **Pausierte Räume werden NIE automatisch gelöscht**, auch nicht nach 24h und auch nicht bei leerem Spielerfeld. Pausierte Räume sind explizit geschützt (`room.paused === true`).
  - **Mid-game laufende Räume** werden nicht abgeräumt, solange noch ein Spieler `connected: true` ist oder eine `leaveDeadline` in der Zukunft liegt.
  - Wenn das Löschen wegen RTDB-Regeln/Netz nicht durchgeht, blendet die Raumliste solche Räume trotzdem aus (`shouldHideRoomInList`), damit sie auf der Startseite nicht stören.
  - Pro Raumcode ein Cooldown von 60s, damit nicht alle Clients gleichzeitig dasselbe löschen.
- **Host-Löschbutton:** Oben in der Topbar (neben Teilen / Code / Verlassen) gibt es für den Host den Button `🗑 Raum löschen`. Für Nicht-Hosts ist der Button ausgeblendet. Vor dem Löschen kommt ein Confirm-Dialog `Möchtest du diesen Raum wirklich löschen? Alle Spieler verlieren den Zugriff.`. Bei Bestätigung wird `rooms/<code>` per `set(…, null)` gelöscht, lokaler State zurückgesetzt und die Join-Ansicht gezeigt.
- **Kein Admin-Passwort.** Bewusst ohne extra Admin-Modus – nur der Host kann löschen, automatisch wird nach den oben genannten Fristen aufgeräumt.
- **RTDB-Regeln:** `database.rules.json` erlaubt das Löschen bereits: die `.validate`-Regel `newData.hasChildren(['hostId','phase']) || !newData.exists()` deckt den Fall ab, dass der gesamte Raumknoten verschwindet (`set(null)` / `remove()`). Es ist nichts an den Regeln zu ändern.

## Neu davor (Pause & Raumliste)
